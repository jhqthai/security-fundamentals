...
{
	int match = 0; // Bool to check if the 2 hash match
...
void generate_digest (char *mess, char *mess_hash, int len, char *digest_method)
{
	...
	md = EVP_get_digestbyname(digest_method); //TODO: There is an error here? its not returning 1 dudeeee
...
int main (int argc, char *argv[])
{
	...
	char *hash_algo = NULL;
	char *mess = NULL ; // Pointer to hold randomly generated characters
  ...
  printf("WE HERE");
	...
	while(1)
	{
		...
		generate_digest(rand_str, rand_hash, 3, hash_algo); // This is computer generate random hash
